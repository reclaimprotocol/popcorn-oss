...
docker run -d --name "$upstream" --network "$network" nginx:alpine >/dev/null
...
echo "route-bound gateway access: active=$active_status expired=$expired_status extended_same_url=$extended_status"
