...
export const ADMIN_SESSION_COOKIE = 'control_plane_admin_session';
export const ADMIN_OAUTH_STATE_COOKIE = 'control_plane_admin_oauth_state';
...
export function readAdminAuthConfig(env: Record<string, string | undefined> = process.env): AdminAuthConfig {
  const strategies = new Set(
    (env.ADMIN_AUTH_STRATEGIES || env.CONTROL_PLANE_ADMIN_AUTH_STRATEGIES || 'password')
      .split(',')
  ...
  return {
    strategies,
    passwordFile: env.ADMIN_PASSWORD_FILE?.trim() || env.CONTROL_PLANE_ADMIN_PASSWORD_FILE?.trim() || undefined,
    legacyUser: env.ADMIN_USER?.trim() || env.CONTROL_PLANE_ADMIN_USER?.trim() || undefined,
    legacyPass: env.ADMIN_PASS?.trim() || env.CONTROL_PLANE_ADMIN_PASS?.trim() || undefined,
    sessionSecret: env.ADMIN_SESSION_SECRET?.trim()
      || env.CONTROL_PLANE_ADMIN_SESSION_SECRET?.trim()
      || env.ADMIN_PASS?.trim()
      || env.CONTROL_PLANE_ADMIN_PASS?.trim()
      || env.ADMIN_TOKEN?.trim()
      || env.CONTROL_PLANE_ADMIN_TOKEN?.trim()
      || undefined,
    sessionTtlSeconds: readPositiveInteger(env.ADMIN_SESSION_TTL_SECONDS || env.CONTROL_PLANE_ADMIN_SESSION_TTL_SECONDS, 12 * 60 * 60),
    bearerToken: env.CONTROL_PLANE_ADMIN_TOKEN?.trim() || env.ADMIN_TOKEN?.trim() || undefined,
    googleClientId: env.ADMIN_GOOGLE_CLIENT_ID?.trim() || env.CONTROL_PLANE_ADMIN_GOOGLE_CLIENT_ID?.trim() || undefined,
    googleClientSecret: env.ADMIN_GOOGLE_CLIENT_SECRET?.trim() || env.CONTROL_PLANE_ADMIN_GOOGLE_CLIENT_SECRET?.trim() || undefined,
...
export async function verifyAdminPassword(username: string, password: string, config: AdminAuthConfig): Promise<boolean> {
  if (!config.strategies.has('password')) return false;
...
export async function authenticateBasicAdmin(header: string | undefined, config: AdminAuthConfig): Promise<AdminIdentity | null> {
  ...
  if (!credentials || !await verifyAdminPassword(credentials.username, credentials.password, config)) {
    return null;
  }
  return { id: credentials.username, displayName: credentials.username, strategy: 'password' };
}

export function authenticateBearerAdmin(header: string | undefined, config: AdminAuthConfig): AdminIdentity | null {
  const token = parseBearerToken(header);
  if (!token || !config.bearerToken || !constantTimeEqual(token, config.bearerToken)) {
    return null;
  }
  return { id: 'admin-token', displayName: 'Admin Token', strategy: 'token' };
}
...
export function verifyAdminSession(cookie: string | undefined, config: AdminAuthConfig, now = Date.now()): AdminIdentity | null {
  ...
  try {
    ...
    return { id: payload.id, displayName: payload.displayName, strategy: payload.strategy };
  } catch {
    return null;
  }
}
...
export function authorizeGoogleUser(userInfo: GoogleUserInfo, config: AdminAuthConfig): AdminIdentity | null {
  ...
  if (!config.googleAllowedEmails.has(email) && (!domain || !config.googleAllowedDomains.has(domain))) {
    return null;
  }
...
export function isSameOriginAdminRequest(request: Request): boolean {
  ...
  const expectedHosts = new Set([
    ...
    url.host,
  ].filter(Boolean).map((host) => normalizeHost(host!)));

  if (!origin && !referer) return false;
  try {
    const source = new URL(origin || referer!);
    return expectedHosts.has(normalizeHost(source.host));
  } catch {
    return false;
  }
}
...
function normalizeHost(value: string): string {
  try {
    return new URL(`https://${value.trim().toLowerCase()}`).hostname;
  } catch {
