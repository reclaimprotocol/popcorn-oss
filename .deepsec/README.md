...
Currently configured project: `popcorn` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/popcorn/SETUP.md` to fill in
   `data/popcorn/INFO.md`.
...
`--project-id popcorn` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/popcorn/`.
