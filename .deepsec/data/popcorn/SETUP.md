# Agent setup for `popcorn`

This is a deepsec scanning workspace. Project `popcorn` was just registered
(target: `..`). Setup is incomplete — `data/popcorn/INFO.md`
...
2. **Fill in `data/popcorn/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id popcorn
pnpm deepsec process --project-id popcorn
```
